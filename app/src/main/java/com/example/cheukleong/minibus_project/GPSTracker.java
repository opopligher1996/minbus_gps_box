...
public class GPSTracker extends Service
{
    ...
    private class LocationListener implements android.location.LocationListener
    {
        ...
        public void onLocationChanged(Location location)
        {
            ...
            Log.e("DEBUG FOR TIMESTAMP:",String.valueOf(set_time()));
            if(count==0)
            {
                journeyid = String.valueOf(set_time())+CAR_ID;
                Location start_pt = new Location(location);
                Location end_pt = new Location(location);
                Location cur = new Location(location);
                cur.setLatitude(endlocation_x);
                cur.setLongitude(endlocation_y);
                start_pt.setLatitude(22.2837);
                start_pt.setLongitude(114.1588);
                end_pt.setLatitude(22.2619);
                end_pt.setLongitude(114.1319);
                float start_dis = cur.distanceTo(cur);
                float end_dis = cur.distanceTo(cur);
                start_time = set_time();
    ...
    try {
        Log.d("Send_i",String.valueOf(send_i));
    ...
    private void send_endpoint_data(int station){
        ...
        Log.d(TAG, "send_station_data: 5");
        try {
            int temp_i = station+5*go_back;
            // Add your data
            List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
            nameValuePairs.add(new BasicNameValuePair("stationid", journeyid+String.valueOf(station)));
            nameValuePairs.add(new BasicNameValuePair("location", endlocation.toString()));
            nameValuePairs.add(new BasicNameValuePair("startTime", start_time.toString()));
    ...
    private void send_location(int route){
        //To Do: Insert journey table
        int route_confirm = 1;
        if(route==1 || route==4)
        {
        ...
        try {
            ...
            nameValuePairs.add(new BasicNameValuePair("journeyid", journeyid));
            nameValuePairs.add(new BasicNameValuePair("routeid", String.valueOf(route_confirm)));
